...
struct SettingsDefaults
{
    ...
    static constexpr double k_waypoint_record_period_s = 1.0 / k_waypoint_record_rate_hz;
...
struct RuntimeProfile
{
    ...
    double max_approach_speed_deg_s = SettingsDefaults::k_max_approach_speed_deg_s;
    ...
    bool enable_realtime_scheduler = SettingsDefaults::k_enable_realtime_scheduler;
    int realtime_priority = SettingsDefaults::k_realtime_priority;

    double loop_period_s() const noexcept
    {
        return 1.0 / loop_rate_hz;
    }
                     ...
                     SafetyState &state, std::string &reason)
{
    if (drv.has_temperature_feedback())
    {
        ...
        {
            reason = "over-temperature (motor=" + std::to_string(motor_t) + "C, drive=" + std::to_string(drive_t) + "C)";
            return true;
        }

        if (hottest >= profile.temp_warn_celsius)
        {
            ECWRN("Joint '%s' warm: motor=%dC drive=%dC\n", joint_name.c_str(), static_cast<int>(motor_t),
                  static_cast<int>(drive_t));
        }
    }
...
{
    ecp::rt_app_t::instance().terminate();
}
...
std::string capability_fix_command(const std::string &executable_path)
{
    return "sudo setcap cap_net_raw,cap_net_admin,cap_sys_nice+ep " + executable_path;
}
...
bool ensure_runtime_capabilities(const std::string &executable_path, const RuntimeProfile &profile)
{
    ...
    std::fprintf(stderr, "Or use the automated project task: pixi run capabilities\n");
    return false;
}
...
void populate_common_fields(JointHandle &jh, const ecp::DeviceConfig &cfg, const std::string &device_name,
                            const std::string &driver_name)
{
    ...
    jh.operation_mode_name = cfg.get<std::string>(device_name, "operation_mode", "");
    ...
    {
        jh.selectable = false;
        jh.unavailable_reason = "unknown operation_mode='" + jh.operation_mode_name + "'";
        return;
    }
...
{
public:
    MyActuatorAdapter(std::shared_ptr<ecp::MyActuator::Driver> driver, uint16_t encoder_bits)
        : m_driver(std::move(driver)), m_encoder_bits(encoder_bits)
    {
    }

    DriverKind kind() const noexcept override
    {
        return DriverKind::MyActuator;
    }

    const char *kind_name() const noexcept override
    {
        return "MT_Device";
    }
    ...
    uint16_t status() const noexcept override
    {
        return m_driver->status();
    }

    bool fault() const noexcept override
    {
        return m_driver->fault();
    }
    ...
    bool has_temperature_feedback() const noexcept override
    {
        return true;
    }
    ...
    uint16_t encoder_bits() const noexcept override
    {
        return m_encoder_bits;
    }
...
{
public:
    NovantaAdapter(std::shared_ptr<ecp::Novanta::Driver> driver, uint16_t encoder_bits)
        : m_driver(std::move(driver)), m_encoder_bits(encoder_bits)
    {
    }
...
public:
    ...
    std::optional<JointHandle> create(const ecp::EthercatBus &bus, const ecp::DeviceConfig &cfg,
                                      const std::string &device_name) const override
    {
        auto d = bus.get_device<ecp::MyActuator::Driver>(device_name);
        if (!d)
        {
            std::cerr << "  skip '" << device_name << "': MT_Device not present on bus\n";
            return std::nullopt;
        }

        JointHandle jh;
        populate_common_fields(jh, cfg, device_name, std::string(driver_name()));
        jh.encoder_bits = cfg.get<int>(device_name, "encoder_bits", d->encoder_bits());
        jh.driver = std::make_shared<MyActuatorAdapter>(d, static_cast<uint16_t>(jh.encoder_bits));
...
public:
                                      ...
                                      const std::string &device_name) const override
    {
        ...
        if (!d)
        {
            std::cerr << "  skip '" << device_name << "': CapitanDrv not present on bus\n";
            return std::nullopt;
        }
        ...
        jh.driver = std::make_shared<NovantaAdapter>(d, static_cast<uint16_t>(jh.encoder_bits));
...
std::vector<std::size_t> pick_joints(const std::vector<JointHandle> &joints)
{
    ...
    for (std::size_t i = 0; i < joints.size(); ++i)
    {
        ...
        std::printf("  [%2zu] %-16s drv=%-10s alias=%-5u model=%-10s mode=%-6s enc=%2dbit KP=%-6d KD=%-5d  %s\n", i,
                    j.name.c_str(), j.driver_name.c_str(), j.alias, j.model.empty() ? "(default)" : j.model.c_str(),
                    j.operation_mode_name.c_str(), j.encoder_bits, j.pvt_kp, j.pvt_kd,
                    j.selectable ? "OK" : ("UNAVAILABLE: " + j.unavailable_reason).c_str());
    }

    std::printf("\nEnter joint index(es) to tune (single '3' or comma-separated '0,2,5'),\n"
                "or 'q' to quit: ");
    std::fflush(stdout);
        ...
        try
        {
            ...
            if (!joints[static_cast<std::size_t>(idx)].selectable)
            {
                std::cerr << "Joint '" << joints[static_cast<std::size_t>(idx)].name
                          << "' is not selectable: " << joints[static_cast<std::size_t>(idx)].unavailable_reason
                          << "\n";
                local_bad = true;
                return;
            }
...
{
    std::printf("\n--- Trajectory mode ---\n");
    std::printf("  [s] sin       -- fixed-frequency sinusoid about the captured mid-point\n");
    std::printf("  [l] chirp-lin -- linear frequency sweep f0->f1->f0 (bode/sweep ID)\n");
    std::printf("  [g] chirp-log -- logarithmic frequency sweep (wide-band ID)\n");
    std::printf("  [t] triangle  -- constant-velocity triangle sweep (friction/range)\n");
    std::printf("  [e] step      -- square-wave step response\n");
    std::printf("  [m] multisine -- summed harmonics, Schroeder phasing (one-shot FRF)\n");
    std::printf("  [p] spline    -- replay a Catmull-Rom spline through waypoints you record\n");
    std::printf("                   by backdriving the joint(s) during idle\n");
    std::printf("\nEnter a key (Enter = keep previous '%s'), or 'q' to quit: ", mode_to_string(previous));
    std::fflush(stdout);
...
SinTrajectory::SinTrajectory(double centre_counts, double amp_counts, double freq_hz, double safety_factor,
                                                         int encoder_bits) noexcept
        : m_centre(centre_counts), m_amp(amp_counts), m_freq(freq_hz), m_safety_factor(safety_factor),
            m_enc_bits(encoder_bits)
{
}

double SinTrajectory::approach_target() const noexcept
{
    return m_centre;
}
...
void SinTrajectory::describe_csv(std::ostream &os) const
{
    ...
    os << "# sin_centre_deg, " << counts2deg(static_cast<int32_t>(m_centre), m_enc_bits) << "\n";
...
ChirpTrajectory::ChirpTrajectory(double centre_counts, double amp_counts, double f0_hz, double f1_hz,
                                 double sweep_seconds, bool logarithmic, int encoder_bits) noexcept
    : m_centre(centre_counts), m_amp(amp_counts), m_f0(std::max(1e-6, f0_hz)), m_f1(std::max(1e-6, f1_hz)),
      m_sweep(std::max(1e-3, sweep_seconds)), m_log(logarithmic), m_enc_bits(encoder_bits)
{
}
...
void ChirpTrajectory::describe_csv(std::ostream &os) const
{
    ...
    os << "# chirp_centre_deg, " << counts2deg(static_cast<int32_t>(m_centre), m_enc_bits) << "\n";
    os << "# chirp_amp_deg, " << counts2deg(static_cast<int32_t>(m_amp), m_enc_bits) << "\n";
    os << "# chirp_f0_hz, " << m_f0 << "\n";
    os << "# chirp_f1_hz, " << m_f1 << "\n";
...
TriangleTrajectory::TriangleTrajectory(double centre_counts, double amp_counts, double cycle_seconds,
                                       int encoder_bits) noexcept
    : m_centre(centre_counts), m_amp(amp_counts), m_cycle(std::max(1e-3, cycle_seconds)), m_enc_bits(encoder_bits)
{
}

double TriangleTrajectory::approach_target() const noexcept
{
    return m_centre;
}
...
void TriangleTrajectory::describe_csv(std::ostream &os) const
{
    ...
    os << "# triangle_centre_deg, " << counts2deg(static_cast<int32_t>(m_centre), m_enc_bits) << "\n";
    os << "# triangle_amp_deg, " << counts2deg(static_cast<int32_t>(m_amp), m_enc_bits) << "\n";
...
StepTrajectory::StepTrajectory(double centre_counts, double amp_counts, double cycle_seconds, int encoder_bits) noexcept
    : m_centre(centre_counts), m_amp(amp_counts), m_cycle(std::max(1e-3, cycle_seconds)), m_enc_bits(encoder_bits)
{
}
...
void StepTrajectory::describe_csv(std::ostream &os) const
{
    ...
    os << "# step_centre_deg, " << counts2deg(static_cast<int32_t>(m_centre), m_enc_bits) << "\n";
    os << "# step_amp_deg, " << counts2deg(static_cast<int32_t>(m_amp), m_enc_bits) << "\n";
...
MultisineTrajectory::MultisineTrajectory(double centre_counts, double amp_counts, double base_freq_hz, int harmonics,
                                         int encoder_bits)
    : m_centre(centre_counts), m_amp(amp_counts), m_base_freq(std::max(1e-6, base_freq_hz)),
      m_harmonics(std::max(1, harmonics)), m_enc_bits(encoder_bits)
{
    ...
    for (int k = 1; k <= m_harmonics; ++k)
    {
        m_phases[static_cast<std::size_t>(k - 1)] =
            -k_pi * static_cast<double>(k) * static_cast<double>(k - 1) / static_cast<double>(m_harmonics);
    }
}
...
double MultisineTrajectory::sample(double t_s) const noexcept
{
    ...
    for (int k = 1; k <= m_harmonics; ++k)
    {
        ...
        acc += std::sin(2.0 * k_pi * f * t_s + m_phases[static_cast<std::size_t>(k - 1)]);
    }
...
void MultisineTrajectory::describe_csv(std::ostream &os) const
{
    ...
    os << "# multisine_centre_deg, " << counts2deg(static_cast<int32_t>(m_centre), m_enc_bits) << "\n";
    os << "# multisine_amp_deg, " << counts2deg(static_cast<int32_t>(m_amp), m_enc_bits) << "\n";
                                                       ...
                                                       const RuntimeProfile &profile, int encoder_bits)
{
    ...
    {
    case TrajectoryMode::Sin:
        return std::make_unique<SinTrajectory>(centre, amp, profile.traj_freq_hz, profile.traj_safety_factor,
                                               encoder_bits);
    case TrajectoryMode::ChirpLinear:
        return std::make_unique<ChirpTrajectory>(centre, amp, profile.chirp_f0_hz, profile.chirp_f1_hz,
                                                 profile.chirp_sweep_seconds, false, encoder_bits);
    case TrajectoryMode::ChirpLog:
        return std::make_unique<ChirpTrajectory>(centre, amp, profile.chirp_f0_hz, profile.chirp_f1_hz,
                                                 profile.chirp_sweep_seconds, true, encoder_bits);
    case TrajectoryMode::Triangle:
        return std::make_unique<TriangleTrajectory>(centre, amp, profile.triangle_cycle_seconds, encoder_bits);
    case TrajectoryMode::Step:
        return std::make_unique<StepTrajectory>(centre, amp, profile.step_cycle_seconds, encoder_bits);
    case TrajectoryMode::Multisine:
        return std::make_unique<MultisineTrajectory>(centre, amp, profile.multisine_base_hz,
                                                     profile.multisine_harmonics, encoder_bits);
    case TrajectoryMode::Spline:
        break; // Spline is built from recorded waypoints, not here.
    }
    return std::make_unique<SinTrajectory>(centre, amp, profile.traj_freq_hz, profile.traj_safety_factor, encoder_bits);
}

SplineTrajectory::SplineTrajectory(std::vector<double> knot_times, std::vector<int32_t> waypoint_counts, int encoder_bits)
    : m_t(std::move(knot_times)), m_w(waypoint_counts.begin(), waypoint_counts.end()), m_enc_bits(encoder_bits)
{
...
double SplineTrajectory::sample(double t_s) const noexcept
{
    ...
    return h00 * m_w[i] + h10 * dt * m_m[i] + h01 * m_w[i + 1] + h11 * dt * m_m[i + 1];
}
...
double SplineTrajectory::total_one_way_seconds() const noexcept
{
    return m_T;
}
...
bool mkdirs_recursive(const std::string &path)
{
    ...
    for (char ch : path)
    {
        ...
        if (ch == '/')
        {
            if (partial.size() > 1 && !mkdir_if_needed(partial.substr(0, partial.size() - 1)))
            {
                return false;
            }
        }
...
JointCsvLogger::~JointCsvLogger()
{
    close();
}

bool JointCsvLogger::open(const std::string &path, const RuntimeProfile &profile, const JointHandle &jh,
                         const JointPlan &plan, const Trajectory &traj)
{
    ...
    std::fprintf(m_file,
                 ...
                 "ref_raw_deg,ref_filt_deg,actual_deg,motor_temp_c,drive_temp_c,error_code\n");
...
const std::string &JointCsvLogger::path() const noexcept
{
    return m_path;
}

bool JointCsvLogger::is_open() const noexcept
{
...
void JointCsvLogger::write_batch(const std::vector<LogSample> &batch)
{
    ...
    for (const LogSample &s : batch)
    {
        const int written = std::snprintf(
            line, sizeof(line),
            "%.9f,%d,%.9f,%.6f,%.6f,%d,%.9f,%.9f,%.9f,%d,%d,%u\n", s.t_s, s.phase, s.phase_t_s,
            s.ref_raw_counts, s.ref_filt_counts, s.actual_counts,
            ...
            counts2deg(s.actual_counts, m_encoder_bits), static_cast<int>(s.motor_temp_c),
            static_cast<int>(s.drive_temp_c), static_cast<unsigned>(s.error_code));
        if (written > 0)
        {
...
std::string make_run_log_dir(const RuntimeProfile &profile)
{
    ...
    const std::string root_dir = (env_root != nullptr && env_root[0] != '\0') ? std::string(env_root)
                                                                                : profile.log_root_dir;
    if (!mkdirs_recursive(root_dir))
    {
        std::fprintf(stderr, "warning: cannot create log root '%s' (errno=%d) -- logging disabled\n", root_dir.c_str(), errno);
        return {};
    }
    ...
    {
        std::fprintf(stderr, "warning: cannot create log dir '%s' (errno=%d) -- logging disabled\n", dir.c_str(), errno);
        return {};
    }
