...
ConnectionPanel::ConnectionPanel(QString default_config, QWidget *parent) : QWidget(parent)
{
    ...
    connect(m_disconnect_btn, &QPushButton::clicked, this, &ConnectionPanel::disconnectRequested);
    ...
    connect(m_joint_list, &QListWidget::itemChanged, this, [this] { emit selectionChanged(); });
}
...
void ConnectionPanel::setJoints(const std::vector<JointInfo> &joints)
{
    ...
    {
        QString label = QString::fromStdString(j.name) + "  [" + QString::fromStdString(j.driver_name) + ", " +
                        QString::fromStdString(j.operation_mode_name) + "]";
        if (!j.selectable)
        {
            label += tr("  (unavailable: %1)").arg(QString::fromStdString(j.unavailable_reason));
        }
        ...
        item->setCheckState(m_selected_joint_names.contains(joint_name) ? Qt::Checked : Qt::Unchecked);
        if (!j.selectable)
        {
...
QString ConnectionPanel::configPath() const
{
