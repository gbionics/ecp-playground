...
void AxisStatusWidget::paintEvent(QPaintEvent *event)
{
    ...
    p.drawText(5, 25, width() - 10, 18,
               Qt::AlignLeft, QString::number(m_telem.actual_deg, 'f', 1) + " deg");

    // Status indicator
    QColor status_color = m_telem.fault        ? QColor(220, 38, 38)
                          : m_telem.limit_violation ? QColor(234, 179, 8)
                          : m_telem.cia402_state == CiA402State::OperationEnabled ? QColor(34, 197, 94)
                                                                                  : QColor(150, 150, 150);
    ...
    p.drawText(5, height() - 40, width() - 10, 16,
               Qt::AlignCenter, "V:" + QString::number(m_telem.velocity_deg_s, 'f', 1) + " deg/s");
}
...
AxisOverviewPanel::AxisOverviewPanel(QWidget *parent)
    : QWidget(parent)
{
...
void AxisOverviewPanel::updateTelemetry(const std::vector<JointTelemetry> &joints)
{
