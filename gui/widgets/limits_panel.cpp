  ...
  connect(m_joint_combo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this](int idx) {
            if (idx >= 0 && static_cast<std::size_t>(idx) < m_joints.size()) {
              m_min_spin->setValue(
                  m_joints[static_cast<std::size_t>(idx)].min_limit_deg);
              m_max_spin->setValue(
                  m_joints[static_cast<std::size_t>(idx)].max_limit_deg);
            }
          });
  connect(apply, &QPushButton::clicked, this, [this] {
    emit setLimitsRequested(currentJoint(), m_min_spin->value(),
                            m_max_spin->value());
  });
...
void LimitsPanel::setState(ControllerState state) {
  const bool idle_or_capturing = (state == ControllerState::Connected ||
