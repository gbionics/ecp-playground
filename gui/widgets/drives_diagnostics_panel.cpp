    ...
    : QWidget(parent)
{
    ...
    connect(m_joint_combo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &DrivesDiagnosticsPanel::onJointSelectionChanged);
    ...
    m_diagnostics_table->setHorizontalHeaderLabels({tr("Parameter"), tr("Value")});
    m_diagnostics_table->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents);
    m_diagnostics_table->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
...
void DrivesDiagnosticsPanel::updateTelemetry(const std::vector<JointTelemetry> &joints)
{
    if (joints.empty())
        return;
    ...
    QString homing_text = telem.homed ? tr("Homing: Completed") : tr("Homing: Not Completed");
...
{
    int joint_idx = m_joint_combo->currentIndex();
    if (joint_idx < 0 || joint_idx >= static_cast<int>(m_current_telemetry.size()))
        return;
    ...
    addRow(tr("Reference"), QString::number(telem.reference_deg, 'f', 2) + " deg");
    addRow(tr("Following Error"), QString::number(telem.following_error_deg, 'f', 3) + " deg");
    addRow(tr("Velocity"), QString::number(telem.velocity_deg_s, 'f', 2) + " deg/s");
    addRow(tr("Torque"), QString::number(telem.torque_percent, 'f', 1) + "%");
    addRow(tr("Power"), QString::number(telem.power_watts, 'f', 1) + " W");
    addRow(tr("Motor Temperature"), telem.motor_temp_c >= 0 ? QString::number(telem.motor_temp_c) + " °C"
                                                               : tr("--"));
    addRow(tr("Drive Temperature"), telem.drive_temp_c >= 0 ? QString::number(telem.drive_temp_c) + " °C"
                                                               : tr("--"));
    addRow(tr("Soft Limits"), QString::number(telem.soft_min_limit_deg, 'f', 1) + " to " +
                                   QString::number(telem.soft_max_limit_deg, 'f', 1) + " deg");
    addRow(tr("Hard Limits"), QString::number(telem.min_limit_deg, 'f', 1) + " to " +
                                  QString::number(telem.max_limit_deg, 'f', 1) + " deg");
    addRow(tr("Limit Violation"), telem.limit_violation ? tr("YES") : tr("No"));
    addRow(tr("Hard Limit Violation"), telem.hard_limit_violation ? tr("YES") : tr("No"));
    addRow(tr("Fault"), telem.fault ? tr("YES") : tr("No"));
    addRow(tr("Operation Mode"), QString::fromStdString(
               m_joints.size() > joint_idx ? m_joints[joint_idx].operation_mode_name : "Unknown"));
}
