...
EnhancedLimitsPanel::EnhancedLimitsPanel(QWidget *parent) : QWidget(parent) {
  auto *layout = new QVBoxLayout(this);

  // Joint selector
  auto *selector_layout = new QHBoxLayout();
  auto *joint_label = new QLabel(tr("&Joint:"));
  selector_layout->addWidget(joint_label);
  ...
  connect(m_joint_combo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &EnhancedLimitsPanel::onJointChanged);
  ...
  m_capture_btn->setAccessibleName(tr("Toggle backdrive capture"));
  m_capture_btn->setToolTip(tr("Move the joint by hand between its extremes "
                               "while this is on, then stop to apply."));
  connect(m_capture_btn, &QPushButton::toggled, this, [this](bool checked) {
    m_capturing = checked;
    m_capture_btn->setText(checked ? tr("Stop Capture and Apply")
                                   : tr("Start Backdrive Capture"));
    emit captureToggled(checked);
  });
  capture_layout->addWidget(m_capture_btn);
  m_auto_info_label = new QLabel(tr(
      "Move the actuator to physical minimum and maximum while capture is ON.\n"
  ...
  m_hard_min_spin->setAccessibleName(tr("Minimum limit in degrees"));
  ...
  m_hard_max_spin->setAccessibleName(tr("Maximum limit in degrees"));
  hard_limits_layout->addWidget(m_hard_max_spin);

  m_apply_limits_btn = new QPushButton(tr("&Apply Min/Max"));
  m_apply_limits_btn->setAccessibleName(tr("Apply manual limits"));
  m_apply_limits_btn->setToolTip(
      tr("Apply the typed minimum and maximum as the joint's soft limits."));
  connect(m_apply_limits_btn, &QPushButton::clicked, this,
          &EnhancedLimitsPanel::onApplyLimitsClicked);
  hard_limits_layout->addWidget(m_apply_limits_btn);

  m_reset_btn = new QPushButton(tr("&Reset"));
  m_reset_btn->setAccessibleName(tr("Reset limits"));
  m_reset_btn->setToolTip(
      tr("Clear the learned envelope back to the current position."));
  ...
  m_manual_info_label =
      new QLabel(tr("Enter both min and max and press Apply Min/Max."));
...
void EnhancedLimitsPanel::setState(ControllerState state) {
  // Limits may only be captured/overridden while idle or capturing, never
  // mid-motion.
  const bool enabled = (state == ControllerState::Connected ||
                        state == ControllerState::Capturing);
  m_capture_btn->setEnabled(enabled);
  m_apply_limits_btn->setEnabled(enabled);
  m_reset_btn->setEnabled(enabled);
}

void EnhancedLimitsPanel::updateLiveLimits(
  ...
  // Only track the live learned envelope into the spin boxes while a capture is
  // running. Once capture stops, the boxes hold whatever the operator applied
  // so that a manual override is not clobbered by continued auto-learning.
  if (!m_capturing) {
    return;
  }
