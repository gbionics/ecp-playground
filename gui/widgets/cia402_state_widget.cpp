...
CiA402StateWidget::CiA402StateWidget(QWidget *parent)
    : QWidget(parent)
{
...
void CiA402StateWidget::layoutStates()
{
    ...
    m_state_boxes.push_back({"Not Ready", 20, 20, 100, 40, CiA402State::NotReadyToSwitchOn});
    m_state_boxes.push_back({"Switched On\nDisabled", 140, 20, 100, 40, CiA402State::SwitchedOnDisabled});
    m_state_boxes.push_back({"Ready to\nSwitch On", 260, 20, 100, 40, CiA402State::ReadyToSwitchOn});

    // Row 2: SwitchedOn, OperationEnabled
    m_state_boxes.push_back({"Switched On", 140, 100, 100, 40, CiA402State::SwitchedOn});
    m_state_boxes.push_back({"Operation\nEnabled", 260, 100, 100, 40, CiA402State::OperationEnabled});

    // Row 3: QuickStop, FaultReactionActive, Fault
    m_state_boxes.push_back({"Quick Stop", 20, 180, 100, 40, CiA402State::QuickStop});
    m_state_boxes.push_back({"Fault\nReaction", 140, 180, 100, 40, CiA402State::FaultReactionActive});
    m_state_boxes.push_back({"Fault", 260, 180, 100, 40, CiA402State::Fault});
}

CiA402StateWidget::StateBox CiA402StateWidget::stateAt(CiA402State state) const
{
...
QColor CiA402StateWidget::stateColor(CiA402State state, bool active) const
{
    if (active)
    {
        if (state == CiA402State::Fault || state == CiA402State::FaultReactionActive)
