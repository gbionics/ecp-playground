...
const char *to_string(EventType type) noexcept
{
    switch (type)
    {
        ...
        case EventType::LimitViolation: return "Limit Violation";
        case EventType::StateChange: return "State Change";
        case EventType::HardwareStatus: return "Hardware Status";
    }
    return "Unknown";
}
    ...
    : QWidget(parent)
{
    ...
    connect(m_export_btn, &QPushButton::clicked, this, &EventLogPanel::onExportLog);
    ...
    m_log_table->setHorizontalHeaderLabels({tr("Time"), tr("Type"), tr("Joint"), tr("Message"), tr("Value")});
    m_log_table->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents);
    m_log_table->horizontalHeader()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
    m_log_table->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
    m_log_table->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Stretch);
    m_log_table->horizontalHeader()->setSectionResizeMode(4, QHeaderView::ResizeToContents);
...
void EventLogPanel::exportToFile(const QString &path)
{
    ...
    for (const auto &event : m_events)
    {
        stream << event.timestamp.toString("yyyy-MM-dd HH:mm:ss.zzz") << ","
               ...
               << event.joint_name << ","
               << event.message << ","
               << event.associated_value << "\n";
    }

    file.close();
}

void EventLogPanel::onClearLog()
{
    clearLog();
}
...
void EventLogPanel::addEventRow(const SystemEvent &event)
{
    ...
    auto *time_item = new QTableWidgetItem(event.timestamp.toString("HH:mm:ss.zzz"));
    ...
    auto *value_item = new QTableWidgetItem(QString::number(event.associated_value, 'f', 2));
...
QColor EventLogPanel::eventTypeColor(EventType type) const
{
    switch (type)
    {
        case EventType::Info: return QColor(230, 245, 255);
        case EventType::Warning: return QColor(255, 250, 200);
        case EventType::Error: return QColor(255, 200, 200);
        case EventType::Fault: return QColor(220, 20, 60);
        case EventType::LimitViolation: return QColor(255, 200, 150);
        case EventType::StateChange: return QColor(200, 230, 200);
        case EventType::HardwareStatus: return QColor(240, 240, 240);
    }
...
QString EventLogPanel::eventTypeIcon(EventType type) const
{
    switch (type)
    {
        case EventType::Info: return "ℹ";
        case EventType::Warning: return "⚠";
        case EventType::Error: return "✕";
        case EventType::Fault: return "⚡";
        case EventType::LimitViolation: return "◆";
        case EventType::StateChange: return "→";
        case EventType::HardwareStatus: return "◆";
    }
    return "";
}
