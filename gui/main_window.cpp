...
QString tempString(int16_t t) {
  return (t < 0) ? QStringLiteral("--")
                 : QString::number(t) + QStringLiteral("C");
}
...
QString tupleNumbers(const QList<double> &values, int decimals) {
  ...
  return QStringLiteral("(") + out.join(QStringLiteral(" ")) +
         QStringLiteral(")");
}

} // namespace

MainWindow::MainWindow(RuntimeProfile profile, QString default_config,
                       QWidget *parent)
    : QMainWindow(parent), m_profile(profile),
      m_default_config(std::move(default_config)) {
  ...
  m_state_label->setAccessibleName(tr("Controller state"));
  statusBar()->addPermanentWidget(m_state_label);

  m_health_indicator = new QLabel(tr("●"));
  m_health_indicator->setStyleSheet(
      "color: red; font-size: 12px; font-weight: bold;");
  m_health_indicator->setToolTip(tr("System health status"));
  m_health_indicator->setAccessibleName(tr("System health"));
  statusBar()->addPermanentWidget(m_health_indicator);

  m_metrics_label = new QLabel(tr("t=0.00s | selected=0"));
  m_metrics_label->setAccessibleName(tr("Live metrics"));
  statusBar()->addPermanentWidget(m_metrics_label);

  m_record_label = new QLabel(tr("REC 0.0s"));
  m_record_label->setVisible(false);
  m_record_label->setStyleSheet("color:#f87171; font-weight:700;");
  m_record_label->setAccessibleName(tr("Recording elapsed time"));
  statusBar()->addPermanentWidget(m_record_label);

  m_record_btn = new QPushButton(tr("● &Record"));
  m_record_btn->setCheckable(true);
  m_record_btn->setMinimumHeight(30);
  m_record_btn->setMinimumWidth(120);
  m_record_btn->setStyleSheet(
      "QPushButton { background:#1f2937; color:#e5e7eb; font-weight:600; "
      "border:1px solid #374151; "
      "border-radius:6px; padding:4px 10px; }"
      "QPushButton:checked { background:#b91c1c; color:white; "
      "border-color:#7f1d1d; }"
      "QPushButton:disabled { background:#374151; color:#9ca3af; }");
  m_record_btn->setToolTip(tr("Record all joints to CSV in any state (manual "
                              "moves, holds, backdriving). "
                              "Shortcut: Ctrl+R"));
  m_record_btn->setAccessibleName(tr("Record telemetry to CSV"));
  m_record_btn->setAccessibleDescription(
      tr("Toggle free-running CSV recording of every joint, independent of "
         "trajectory playback."));
  statusBar()->addPermanentWidget(m_record_btn);

  m_estop_btn = new QPushButton(tr("EMERGENCY STOP"));
  m_estop_btn->setMinimumHeight(36);
  m_estop_btn->setMinimumWidth(220);
  m_estop_btn->setStyleSheet("QPushButton { background:#c1121f; color:white; "
                             "font-weight:700; border:2px solid #7f1d1d; "
                             "border-radius:6px; padding:6px 12px; }"
                             "QPushButton:pressed { background:#9b0d18; }"
                             "QPushButton:disabled { background:#6b7280; "
                             "border-color:#4b5563; color:#e5e7eb; }");
  m_estop_btn->setToolTip(tr("Immediately stop all motion and idle all drives. "
                             "Shortcut: Ctrl+Shift+E"));
  m_estop_btn->setAccessibleName(tr("Emergency stop"));
  m_estop_btn->setAccessibleDescription(
      tr("Immediately abort all motion, stop any recording target, and idle "
         "every drive."));
  ...
  m_store_homing_btn->setToolTip(
      tr("Save current offsets/limits to build/joint-offsets.xml"));
  m_store_homing_btn->setAccessibleName(tr("Store homing snapshot"));
  ...
  m_shortcut_play =
      new QShortcut(QKeySequence(QStringLiteral("Ctrl+Return")), this);
  m_shortcut_record =
      new QShortcut(QKeySequence(QStringLiteral("Ctrl+R")), this);

  m_timer = new QTimer(this);
  m_timer->setInterval(
      16); // Main poll loop - still 60 Hz but delegates to scheduler
...
void MainWindow::buildMenu() {
  ...
  auto *export_telemetry =
      file_menu->addAction(tr("Export Telemetry Snapshot CSV..."));
  file_menu->addSeparator();
  auto *open_logs = file_menu->addAction(tr("Open Logs && Recordings Folder"));

  connect(save_preset, &QAction::triggered, this,
          &MainWindow::savePresetToFile);
  connect(load_preset, &QAction::triggered, this,
          &MainWindow::loadPresetFromFile);
  connect(export_xml, &QAction::triggered, this, &MainWindow::exportOffsetsXml);
  connect(export_telemetry, &QAction::triggered, this,
          &MainWindow::exportTelemetrySnapshot);
  ...
  connect(open_logs, &QAction::triggered, this, [this] {
    QString root = QString::fromStdString(m_profile.log_root_dir);
    QDir dir(root);
    QString abs = dir.isAbsolute()
                      ? dir.absolutePath()
...
void MainWindow::buildDocks() {
  auto addDock = [this](const QString &title, QWidget *w,
                        Qt::DockWidgetArea area, bool tabify = false) {
  ...
  auto *jog_dock = addDock(tr("Jog / Move"), m_jog, Qt::LeftDockWidgetArea);
  auto *lim_dock =
      addDock(tr("Limits"), m_enhanced_limits, Qt::LeftDockWidgetArea);
  auto *traj_dock =
      addDock(tr("Trajectory"), m_trajectory, Qt::LeftDockWidgetArea);

  tabifyDockWidget(jog_dock, lim_dock);
  tabifyDockWidget(lim_dock, traj_dock);
  jog_dock->raise();
  ...
  auto *overview_dock =
      addDock(tr("Axis Overview"), m_axis_overview, Qt::RightDockWidgetArea);
  auto *diag_dock = addDock(tr("Drive Diagnostics"), m_drives_diagnostics,
  ...
  m_table->setHorizontalHeaderLabels(
      {tr("Joint"), tr("Ref [deg]"), tr("Actual [deg]"), tr("Error [deg]"),
  ...
  m_profiler_table->setHorizontalHeaderLabels(
      {tr("Joint"), tr("RMS Err [deg]"), tr("Peak Err [deg]"),
       tr("Peak Vel [deg/s]"), tr("Range Used [deg]"), tr("Samples")});
  m_profiler_table->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
  m_profiler_table->setEditTriggers(QAbstractItemView::NoEditTriggers);
  auto *profiler_dock =
      addDock(tr("Profiler"), m_profiler_table, Qt::BottomDockWidgetArea);
  ...
  auto *event_dock =
      addDock(tr("Event Log"), m_event_log, Qt::BottomDockWidgetArea);

  tabifyDockWidget(profiler_dock, log_dock);
  tabifyDockWidget(log_dock, event_dock);
  profiler_dock->raise();

  // Give the docks sensible starting proportions so controls are readable and
  // the central plot keeps the majority of the space.
  resizeDocks({conn_dock, overview_dock}, {360, 340}, Qt::Horizontal);
  resizeDocks({telemetry_dock}, {250}, Qt::Vertical);

  // === VIEW MENU: dock visibility toggles ===
  if (m_view_menu) {
    for (auto *dock :
         {conn_dock, jog_dock, lim_dock, traj_dock, overview_dock, diag_dock,
          telemetry_dock, profiler_dock, log_dock, event_dock}) {
...
void MainWindow::initializeUpdateScheduler() {
  ...
  m_update_scheduler_id_diagnostics =
      m_scheduler->registerPanel("Diagnostics", 100, [this]() {
  ...
  m_update_scheduler_id_telemetry =
      m_scheduler->registerPanel("Telemetry", 100, [this]() {
...
void MainWindow::wireSignals() {
  ...
  connect(m_record_btn, &QPushButton::toggled, this,
          [this](bool on) { m_worker->post(RecordCommand{on}); });
  ...
  connect(m_shortcut_record, &QShortcut::activated, this, [this] {
    if (m_record_btn->isEnabled()) {
      m_record_btn->toggle();
    }
  });
}
...
void MainWindow::poll() {
  for (const auto &ev : m_worker->drainEvents()) {
    ...
    case WorkerEvent::Kind::Error:
                ...
                QString::fromStdString(ev.message));
      {
        ...
        if (msg.contains(QStringLiteral("record"), Qt::CaseInsensitive)) {
  ...
  // recording on its own, e.g. on disconnect).
  if (m_record_btn->isChecked() != frame.recording) {
    const QSignalBlocker blocker(m_record_btn);
    m_record_btn->setChecked(frame.recording);
  }
  m_record_label->setVisible(frame.recording);
  if (frame.recording) {
    m_record_label->setText(
        tr("● REC %1s")
            .arg(QString::number(frame.recording_elapsed_s, 'f', 1)));
  }

  // Detect recording start/stop edges to give the user unambiguous feedback
  // about where the CSV files are written.
  if (frame.recording && !m_was_recording) {
    m_last_record_dir = QString::fromStdString(frame.recording_dir);
    appendLog(tr("● Recording started → %1").arg(m_last_record_dir));
    statusBar()->showMessage(tr("Recording to %1").arg(m_last_record_dir),
                             4000);
  } else if (!frame.recording && m_was_recording) {
    appendLog(tr("■ Recording stopped."));
    if (!m_last_record_dir.isEmpty()) {
      QMessageBox box(this);
      box.setIcon(QMessageBox::Information);
      box.setWindowTitle(tr("Recording saved"));
      box.setText(tr("Recording saved to:\n%1").arg(m_last_record_dir));
      auto *open_btn =
          box.addButton(tr("Open Folder"), QMessageBox::AcceptRole);
      box.addButton(QMessageBox::Close);
      box.exec();
      if (box.clickedButton() == open_btn) {
        QDesktopServices::openUrl(QUrl::fromLocalFile(m_last_record_dir));
      }
    }
  }
  m_was_recording = frame.recording;

  m_metrics_label->setText(tr("t=%1s | joints=%2 | selected=%3")
  ...
  for (int i = 0; i < static_cast<int>(joints.size()); ++i) {
    m_table->setItem(i, 0,
                     new QTableWidgetItem(QString::fromStdString(
                         joints[static_cast<std::size_t>(i)].name)));
  }
}
...
void MainWindow::updateTelemetry(const TelemetryFrame &frame) {
  ...
  for (int i = 0; i < static_cast<int>(frame.joints.size()); ++i) {
    ...
    setCell(9,
            j.fault ? QStringLiteral("FAULT") : QString::number(j.error_code));
  }
}
...
void MainWindow::updateProfiler(const TelemetryFrame &frame) {
  ...
  for (int i = 0; i < static_cast<int>(frame.joints.size()); ++i) {
    ...
    const double rms =
        (st.samples > 0)
            ? std::sqrt(st.sum_err_sq / static_cast<double>(st.samples))
            : 0.0;
...
void MainWindow::applyPreset(const SessionPreset &preset) {
  ...
  m_trajectory->setSelectedMode(
      static_cast<TrajectoryMode>(preset.trajectory_mode));
...
void MainWindow::savePresetToFile() {
  const QString path = QFileDialog::getSaveFileName(
      this, tr("Save session preset"), QStringLiteral("session-preset.json"),
      tr("Preset JSON (*.json)"));
  if (path.isEmpty()) {
    return;
  }
  ...
  if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
    QMessageBox::warning(this, tr("Preset save failed"),
                         tr("Cannot write preset file."));
    return;
  }
...
void MainWindow::loadPresetFromFile() {
  const QString path = QFileDialog::getOpenFileName(
      this, tr("Load session preset"), QString(), tr("Preset JSON (*.json)"));
  if (path.isEmpty()) {
    return;
  }
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    QMessageBox::warning(this, tr("Preset load failed"),
                         tr("Cannot open preset file."));
    return;
  }
  ...
  if (err.error != QJsonParseError::NoError || !doc.isObject()) {
    QMessageBox::warning(this, tr("Preset load failed"),
                         tr("Invalid preset JSON."));
    return;
  }

  const QJsonObject root = doc.object();
  SessionPreset preset;
  preset.config_path = root.value(QStringLiteral("config_path"))
                           .toString(m_connection->configPath());
  const QJsonArray names =
      root.value(QStringLiteral("selected_joint_names")).toArray();
  for (const auto &n : names) {
    if (n.isString()) {
      preset.selected_joint_names << n.toString();
    }
  }
  preset.trajectory_mode = root.value(QStringLiteral("trajectory_mode"))
                               .toInt(static_cast<int>(TrajectoryMode::Sin));
  preset.logging_enabled =
      root.value(QStringLiteral("logging_enabled")).toBool(true);
...
void MainWindow::exportOffsetsXml() {
  const QString path = QFileDialog::getSaveFileName(
      this, tr("Export joint offsets XML"), QStringLiteral("joint-offsets.xml"),
      tr("XML file (*.xml)"));
  if (path.isEmpty()) {
    return;
  }
...
void MainWindow::exportTelemetrySnapshot() {
  const TelemetryFrame frame = m_worker->snapshot();
  if (frame.joints.empty()) {
    QMessageBox::information(this, tr("Export Telemetry"),
                             tr("No telemetry available. Connect first."));
    return;
  }

  const QString suggested = QStringLiteral("telemetry-%1.csv")
                                .arg(QDateTime::currentDateTime().toString(
                                    QStringLiteral("yyyyMMdd-HHmmss")));
  const QString path = QFileDialog::getSaveFileName(
      this, tr("Export telemetry snapshot"), suggested, tr("CSV file (*.csv)"));
  if (path.isEmpty()) {
    return;
  }
  ...
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    QMessageBox::warning(this, tr("Export Telemetry"),
                         tr("Could not write to %1").arg(path));
    return;
  }

  QTextStream out(&file);
  out << "joint,reference_deg,actual_deg,error_deg,velocity_deg_s,ref_velocity_"
         "deg_s,"
  ...
  if (m_log) {
    m_log->appendPlainText(tr("Telemetry snapshot exported to %1").arg(path));
  }
}
...
void MainWindow::exportOffsetsXmlToPath(const QString &path, bool silent) {
  ...
  if (joints.empty()) {
    QMessageBox::information(this, tr("Export XML"),
                             tr("No joints available. Connect first."));
    return;
  }
  ...
  if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
    QMessageBox::warning(this, tr("Export XML failed"),
                         tr("Cannot write XML file."));
    return;
  }
  ...
  out << "  <param name=\"position_min\">" << tupleNumbers(mins, 3)
      << "</param>\n";
  out << "  <param name=\"position_max\">" << tupleNumbers(maxs, 3)
      << "</param>\n";
  ...
  if (!silent) {
    QMessageBox::information(this, tr("Homing Stored"),
                             tr("Joint offsets exported to:\n%1").arg(path));
  }
}
