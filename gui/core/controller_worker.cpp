...
/// DS402 status word bits: 0-3 (state), 4 (voltage enabled), 5 (quick stop), 6
/// (enable), 7 (fault), 14 (warning), 15 (manufacturer)
CiA402State map_status_to_cia402_state(uint16_t status_word,
                                       bool fault) noexcept {
  if (fault)
...
std::vector<int32_t> decimate_trail(const std::vector<int32_t> &trail,
                                    int encoder_bits,
  ...
  for (std::size_t i = 1; i + 1 < trail.size(); ++i) {
    const double d_deg =
        std::fabs(counts2deg(trail[i] - trail[kept.back()], encoder_bits));
  ...
  const double tail_deg =
      std::fabs(counts2deg(trail.back() - trail[kept.back()], encoder_bits));
...
struct ControllerWorker::Impl {
  struct JointCtl {
    ...
    // Free-running recording (independent of any trajectory).
    std::unique_ptr<JointCsvLogger> rec_logger;
    ...
    double prev_ref_deg =
        0.0; ///< Reference angle at previous telemetry publish.
    double prev_ref_t_s =
        -1.0; ///< Time of previous publish (<0 until first sample).
  ...
  // Free-running recording state.
  bool recording = false;
  int64_t rec_start_tick = 0;
  std::string rec_dir;
...
void ControllerWorker::waitForCommand() {
  ...
  m_cmd_cv.wait(lk,
                [this] { return !m_commands.empty() || m_shutdown.load(); });
}
...
void ControllerWorker::run() {
  ...
  while (!m_shutdown.load()) {
    ...
    std::this_thread::sleep_until(
        impl.t0 + std::chrono::microseconds(static_cast<int64_t>(
                      impl.tick * m_profile.loop_period_s() * 1e6)));
  }
...
void ControllerWorker::handleCommand(const ConnectCommand &c) {
  ...
  if (impl.joints.empty()) {
    error("no supported joints matched. Supported drivers: " +
  ...
  for (std::size_t i = 0; i < impl.joints.size(); ++i) {
    ...
    if (jh.driver) {
      ...
      ct.activity =
          Activity::Capture; // auto-track the range as it is moved by hand
    }
  }
  ...
  publishJointInfo();
  log("connected: " + std::to_string(impl.joints.size()) + " joint(s) on '" +
      ifname + "'");
  log("backdrive each joint through its range \u2014 limits update live; "
      "jogging unlocks once a range is captured");
...
void ControllerWorker::handleCommand(const GoToCommand &c) {
  ...
  const double lim_min =
      soft_valid ? ct.soft_min_counts : static_cast<double>(ct.min_counts);
  const double lim_max =
      soft_valid ? ct.soft_max_counts : static_cast<double>(ct.max_counts);
  target = std::clamp(target, lim_min, lim_max);

  const double start = ct.cmd_counts;
  const double speed = (c.speed_deg_s > 0.0)
                           ? c.speed_deg_s
                           : m_profile.max_approach_speed_deg_s;
  const double dist_deg =
      std::fabs(counts2deg(static_cast<int32_t>(target - start), bits));
...
void ControllerWorker::handleCommand(const StartTrajectoryCommand &c) {
  ...
  for (std::size_t j : c.joints) {
    ...
    if (mode_requires_recording(c.mode)) {
      std::vector<int32_t> waypoints =
          decimate_trail(ct.trail_counts, bits, m_profile);
      if (waypoints.size() < m_profile.min_spline_waypoints) {
        error(
            "'" + jh.name +
            "': not enough recorded motion for a spline (need >= " +
      ...
      for (std::size_t k = 1; k < waypoints.size(); ++k) {
        const double d_deg =
            std::fabs(counts2deg(waypoints[k] - waypoints[k - 1], bits));
        const double seg = std::max(m_profile.min_segment_seconds,
                                    d_deg / m_profile.max_approach_speed_deg_s);
        knot_times[k] = knot_times[k - 1] + seg;
      }
      const std::size_t n_wp = waypoints.size();
      ct.traj = std::make_unique<SplineTrajectory>(std::move(knot_times),
                                                   std::move(waypoints), bits);
      log("'" + jh.name + "': spline with " + std::to_string(n_wp) +
          " waypoint(s) from " + std::to_string(ct.trail_counts.size()) +
          " recorded samples");
    } else {
      ...
      const double centre =
          static_cast<double>((ct.min_counts + ct.max_counts) / 2);
      const double amp =
          m_profile.traj_safety_factor *
          static_cast<double>((ct.max_counts - ct.min_counts) / 2);
      ct.traj =
          make_parametric_trajectory(c.mode, centre, amp, m_profile, bits);
    }

    engageJoint(j);
    ct.start_counts = static_cast<double>(jh.driver->actual_position());
    const double start_deg =
        counts2deg(static_cast<int32_t>(ct.start_counts), bits);
    const double target_deg =
        counts2deg(static_cast<int32_t>(ct.traj->approach_target()), bits);
    const double dist_deg = std::fabs(target_deg - start_deg);
    ct.approach_T = std::max(m_profile.approach_seconds,
                             dist_deg / m_profile.max_approach_speed_deg_s);
    ct.phase = 0; // Hold
    ct.phase_t = 0.0;
    ct.safety = SafetyState{};
    ct.lpf = std::make_unique<LowPass>(
        m_profile.lpf_cutoff_hz, m_profile.loop_period_s(), ct.start_counts);
...
void ControllerWorker::handleCommand(const StopCommand &) {
  auto &impl = *m_impl;
  // Emergency stop: abort every activity (trajectory, jog, go-to, capture) and
  // leave all drives idled/backdrivable, regardless of the current state.
  for (std::size_t i = 0; i < impl.joints.size(); ++i) {
    idleJoint(i);
  }
  log("emergency stop; all activity aborted, all joints idled");
  if (impl.bus_up) {
    // Force a clean idle state even if we were Faulted, so the operator can
    // recover without having to disconnect/reconnect.
...
void ControllerWorker::handleCommand(const RecordCommand &c) {
  auto &impl = *m_impl;
  if (c.start) {
    if (impl.recording) {
      return; // already recording
    }
    if (!impl.bus_up || impl.joints.empty()) {
      error("cannot record: connect to the bus first");
      return;
    }

    impl.rec_dir = make_run_log_dir(m_profile);
    if (impl.rec_dir.empty()) {
      error("cannot record: log directory unavailable");
      return;
    }
    ...
    for (std::size_t i = 0; i < impl.joints.size(); ++i) {
      ...
      const std::string path = impl.rec_dir + "/" + jh.name + "-recording.csv";
      ct.rec_logger = std::make_unique<JointCsvLogger>();
      if (ct.rec_logger->open_plain(path, m_profile, jh, ct.min_counts,
                                    ct.max_counts,
                                    static_cast<int32_t>(ct.cmd_counts))) {
        ++opened;
      } else {
        ct.rec_logger.reset();
        error("cannot open recording for '" + jh.name + "'");
      }
    }

    if (opened == 0) {
      impl.rec_dir.clear();
      error("recording failed to open any joint file");
      return;
    }

    impl.recording = true;
    impl.rec_start_tick = impl.tick;
    log("recording started -> " + impl.rec_dir);
  } else {
    stopRecording();
  }
}
...
void ControllerWorker::stopRecording() {
  auto &impl = *m_impl;
  if (!impl.recording) {
    return;
  }
  for (auto &ct : impl.ctl) {
    if (ct.rec_logger) {
      if (ct.rec_logger->is_open()) {
        ct.rec_logger->close();
      }
      ct.rec_logger.reset();
    }
  ...
  log("recording stopped -> " + impl.rec_dir);
}
...
void ControllerWorker::controlTick() {
  ...
  for (std::size_t i = 0; i < impl.joints.size(); ++i) {
    ...
    case Activity::Capture: {
      ...
      ct.max_counts = std::max(ct.max_counts, a);
      ct.cmd_counts =
          static_cast<double>(a); // keep command at the hand-held position
      ct.limits_set = (ct.max_counts > ct.min_counts);
      // Record a position trail for spline training.
      ct.trail_t += dt;
      if (ct.trail_counts.empty() ||
          ct.trail_t >= m_profile.waypoint_record_period_s()) {
        ct.trail_t = 0.0;
    ...
    case Activity::Jog: {
      ...
      if (ct.limits_set) {
        const bool soft_valid = (ct.soft_max_counts > ct.soft_min_counts);
        const double lim_min = soft_valid ? ct.soft_min_counts
                                          : static_cast<double>(ct.min_counts);
        const double lim_max = soft_valid ? ct.soft_max_counts
                                          : static_cast<double>(ct.max_counts);
    ...
    case Activity::GoTo: {
      ct.cmd_counts =
          min_jerk(ct.goto_t, ct.goto_T, ct.goto_start, ct.goto_target);
      ct.goto_t += dt;
      d.apply_runtime_gains(jh.pvt_kp, jh.pvt_kd);
      d.set_target_position(static_cast<int32_t>(ct.cmd_counts));
      d.set_target_velocity(0);
    ...
    case Activity::Trajectory: {
      ...
      {
        ref_counts = min_jerk(ct.phase_t, ct.approach_T, ct.start_counts,
                              ct.traj->approach_target());
    ...
    // Free-running recording: capture every joint each tick, whatever the
    // activity, so manual moves / holds / backdriving are all logged.
    if (impl.recording && ct.rec_logger && ct.rec_logger->is_open()) {
      LogSample sample;
      sample.t_s = static_cast<double>(impl.tick - impl.rec_start_tick) * dt;
      sample.phase = static_cast<int>(ct.activity);
      sample.phase_t_s = 0.0;
      ...
      sample.drive_temp_c =
          d.has_temperature_feedback() ? d.drive_temperature() : -1;
      sample.error_code = d.error_code();
      ct.rec_logger->write(sample);
    }
  }
...
void ControllerWorker::publishTelemetry() {
  ...
  frame.recording = impl.recording;
  frame.recording_elapsed_s =
      impl.recording ? static_cast<double>(impl.tick - impl.rec_start_tick) *
                           m_profile.loop_period_s()
                     : 0.0;
  frame.recording_dir = impl.recording ? impl.rec_dir : std::string{};
  ...
  for (std::size_t i = 0; i < impl.joints.size(); ++i) {
    ...
    if (jh.driver) {
      ...
      const int bits = d.encoder_bits();
      jt.reference_deg =
          actuator_test::counts2deg(static_cast<int32_t>(ct.cmd_counts), bits);
      ...
      if (dt > 0.0) {
        jt.acceleration_deg_s2 =
            (jt.velocity_deg_s - ct.prev_velocity_deg_s) / dt;
        ct.prev_velocity_deg_s = jt.velocity_deg_s;
      }

      // Reference velocity: rate of change of the commanded angle between
      // telemetry publishes. Populated for every activity so the plot's
      // reference-velocity trace is meaningful during manual moves too.
      if (ct.prev_ref_t_s >= 0.0) {
        const double d_t = frame.t_s - ct.prev_ref_t_s;
        if (d_t > 1e-6) {
          jt.ref_velocity_deg_s = (jt.reference_deg - ct.prev_ref_deg) / d_t;
        }
      }
      ...
      const bool soft_valid = (ct.soft_max_counts > ct.soft_min_counts);
      const double soft_min_counts =
          soft_valid ? ct.soft_min_counts : static_cast<double>(ct.min_counts);
      const double soft_max_counts =
          soft_valid ? ct.soft_max_counts : static_cast<double>(ct.max_counts);
      jt.soft_min_limit_deg = actuator_test::counts2deg(
          static_cast<int32_t>(soft_min_counts), bits);
      jt.soft_max_limit_deg = actuator_test::counts2deg(
          static_cast<int32_t>(soft_max_counts), bits);

      // Limit violation detection
      jt.hard_limit_violation =
...
void ControllerWorker::publishJointInfo() {
  ...
  for (std::size_t i = 0; i < impl.joints.size(); ++i) {
    ...
    info.min_limit_deg =
        actuator_test::counts2deg(ct.min_counts, info.encoder_bits);
    info.max_limit_deg =
        actuator_test::counts2deg(ct.max_counts, info.encoder_bits);
